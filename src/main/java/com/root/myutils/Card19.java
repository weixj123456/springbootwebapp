...
import com.root.xmlutils.XmlStringUtils;
...
public class Card19 {
	...
	private static int tails(final int min, final int max) {
...
//     String cardNumber = "6212264301002712808";//卡号  
       String name = getNameOfBank(cardNumber);
       String cardname=StringUtils.substringBefore(name, "·");
...
private static String getRandomString(String[] strgroup) {
	int index=(int)(Math.random()*strgroup.length);
	String strradom=strgroup[index];
	return strradom;		
}	

private static String getBankbin() {
	String bankbin=null;
	bankbin=getRandomString(bankBin);
	return bankbin;
}	
...
public static void getbankcode(String file) {
// File directory=new File("abc");
// System.out.println(directory.getAbsolutePath());
 String filepath=System.getProperty("user.dir")+"\\src\\org\\cmis\\common\\tools\\";
 System.out.println(filepath);
 String jsonstr=XmlStringUtils.getXmlString(filepath+file);
 ...
 for(Object map:json.entrySet()) {
	 ...
	 keylist.add((String) ((Map.Entry)map).getKey());
 }
 System.out.println(keylist.toString());
}


private static String getBankno19() {
	String bin=Card19.getBankbin();
	String applAcNo18 = bin + tails(100000000, 999999999)+String.valueOf(tails(100, 999));
	Card19  card=new Card19(applAcNo18);
	String bankcardno=card.getCard();
	return bankcardno;
}

/**
 * Card19.getBankCardName(no).equals("建设银行"));
 * @param bankname
 * @return
 */
public static String getBankName(String bankname) {
	String no=Card19.getBankno19();
	int num=0;
	while(!Card19.getBankCardName(no).equals(bankname)) {
		num++;
		no=Card19.getBankno19();
		if(num==100000) {
			if(!Card19.getBankCardName(no).equals(bankname)) {
				System.out.println("当前未找到相应银行卡号,请确认是否有对应银行名称");
				break;
			}
		}
	}
...
public static void main(String[] args) {
	...
	for(int i=0;i<1000000;i++) {
		...
		Card19  card=new Card19(applAcNo18);
		String bankcardno=card.getCard();
		String bankcardname=Card19.getBankCardName(bankcardno);
		if(bankcardname.equals("建设银行")) {
		   System.out.println(bankcardno+"   "+bankcardname);	
		}	
	}
	*/
	/*
	String no=Card19.getBankno19("");
	while(!Card19.getBankCardName(no).equals("建设银行")) {
		  no=Card19.getBankno19("");
//		if(!Card19.getBankCardName(no).equals("建设银行"));		 	
	}
	*/
	String no=Card19.getBankName("中国建设银行");
    System.out.println(no);
//	getbankcode("bankcode.json");
